...
function PaymentDetails() {
  ...
  const [balance] = useState(500000); // Saldo pengguna
  const navigate = useNavigate();

  // Menentukan kereta yang dipilih (misalnya kereta pertama dari data)
  const selectedTrain = trains[0]; // Ambil data kereta pertama sebagai contoh

  const paymentData = {
    providerName: selectedTrain.providerName, // Menggunakan nama provider dari data train
    location: selectedTrain.route, // Menggunakan rute kereta
    passengers: "2 Dewasa, 1 Anak-anak", // Data penumpang bisa disesuaikan
    totalAmount: `Rp ${selectedTrain.price.toLocaleString()}`, // Menggunakan harga kereta
    totalAmountNumeric: selectedTrain.price // Angka untuk perhitungan saldo
  };
  ...
  const handlePayment = () => {
    if (balance >= paymentData.totalAmountNumeric) {
      ...
      // }, 2000); // Waktu tunggu sebelum navigasi
    } else {
      ...
      {isPaymentSuccessful && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-[#95D2B3] p-6 sm:p-8 rounded-lg text-center max-w-sm w-[90%] sm:w-[600px] shadow-lg">
            ...
            <Link to='/Ticket' className="mt-6 bg-white text-black font-semibold py-2 px-4 rounded-lg" onClick={closePopup}>
              Oke
            </Link>
          </div>
        </div>
